...
### ARCHIVED implement distributed huber regression using surrogate loss with gradient shift.
Distributed_Huber_better_twostep <- function(y, X = NULL, m = 10, robust_consts = c(1,1)){
  N = length(y)
  n = N/m
  n.iter = max(2, log(m))
  univ = is.null(dim(X))
  d = ifelse(univ, 0, ncol(X))
  # constants of tau and kappa before multiplying by \hat\sigma
  tau_const = robust_consts[1]*sqrt(N/(d + log(n) + log(log(m))))
  kappa_const = robust_consts[2]*sqrt(n/(d + log(n) + log(log(m))))
  # index function for assigning the m different machines
  index = function(c,n) ((c-1)*n+1):(c*n)
  # tau update using MAD(residuals)
  tau_update = function(y, X, beta, tau_const){
    if (univ){
      res = y - beta
    }else{
    ...
    return(tau_const*median(abs(res - median(res)))/qnorm(0.75))
  }
  # gradient function for huber loss, this is not the surrogate loss
  grad = function(y, X, beta, tau){
    n = length(y)
  ...
  # mean_y = mean(y[index(1)])
  # sd_y = sd(y[index(1)])
  # y = (y - mean_y)/sd_y
  
  # initialize beta^(0), the local estimator using first machine data.
  if(univ){
    beta.list = list(mean(sapply(1:m, function(x) Huber_GDBB(y[index(x,n)], NULL, tau_const = 'adaptive', robust_const = robust_consts[2])$coefficients)))
  }else{
    # beta.list = list(rowMeans(sapply(1:m, function(x) lm(y[index(x)]~as.matrix(X[index(x),-1]))$coefficients)))
    # beta.list = list(rowMeans(sapply(1:m, function(x) Huber_GDBB(y[index(x)], as.matrix(X[index(x),-1])))))
    # beta.list[[1]][1] = median(sapply(1:m, function(x) huber.mean(y[index(x)]-X[index(x),-1]%*%beta.list[[1]][-1])))
    beta.ini.ols = rowMeans(sapply(1:m, function(x) lm(y[index(x,n)]~as.matrix(X[index(x,n),-1]))$coefficients))
    beta.ini.ols[1] = median(sapply(1:m, function(x) huber.mean(y[index(x,n)]-X[index(x,n),-1]%*%beta.ini.ols[-1])))
    beta.ini.huber = rowMeans(sapply(1:m, function(x) huber.reg(as.matrix(X[index(x,n),-1]), y[index(x,n)], method = 'adaptive')))
    beta.ini.huber[1] = median(sapply(1:m, function(x) huber.mean(y[index(x,n)]-X[index(x,n),-1]%*%beta.ini.huber[-1])))
  }
  # update tau and kappa with median of local scales, use it throughtout.
  # sd_median = median(sapply(1:m, function(x) sd(y[index(x)] - X[index(x),]%*%beta.list[[1]])))
  # tau = tau_const*sd_median
  # kappa = kappa_const*sd_median
  tau = median(sapply(1:m, function(x) tau_update(y[index(x)], X[index(x),], beta.ini.huber, tau_const)))
  # kappa = median(sapply(1:m, function(x) tau_update(y[index(x)], X[index(x),], beta.list[[1]], kappa_const)))
  loss.ols = loss(y, X, beta.ini.ols, tau)
  loss.huber = loss(y, X, beta.ini.huber, tau)
  if (loss.huber>loss.ols){
    tau = median(sapply(1:m, function(x) tau_update(y[index(x)], X[index(x),], beta.ini.ols, tau_const)))
    beta.list = list(beta.ini.ols)
    loss.t_1 = loss(y, X, beta.ini.ols, tau)
  }else{
    beta.list = list(beta.ini.huber)
    loss.t_1 = loss.huber
  }
  kappa = tau_update(y[index(1)], X[index(1),], beta.list[[1]], kappa_const)
  # tau_update(y, X, beta.list[[1]], tau_const)
  
  # Do n.iter times of the one-step estimation with surrogate loss minimization.
  for (tt in 1:n.iter){
    # constant gradient shift that will be fed into Huber_GDBB
    if (univ){
      # g.max.t_1 = ifelse(tt == 1, 1, abs(global_grad))
      global_grad = grad(y, X, beta.list[[tt]], tau)
      # if (abs(global_grad) > 1 && tt == 1) return(mean(sapply(1:m, function(x) Huber_GDBB(y_old[index(x)], 0, robust_const = 1, two.step = two.step.intercept))))
      # if (abs(global_grad) > g.max.t_1 | abs(global_grad) < 1e-5) break
      grad_shift = grad(y[index(1)], X[index(1),], beta.list[[tt]], kappa) - global_grad
    }else{
      # g.max.t_1 = ifelse(tt == 1, 1, max(abs(global_grad[-1])))
      global_grad = grad(y, X, beta.list[[tt]], tau)
      # if (max(abs(global_grad[-1])) > 1 && tt == 1) return(rowMeans(sapply(1:m, function(x) Huber_GDBB(y_old[index(x)], X_old[index(x),], robust_const = 1, two.step = two.step.intercept))))
      # if (max(abs(global_grad[-1])) > g.max.t_1 | max(abs(global_grad[-1])) < 1e-5) break
      grad_shift = grad(y[index(1)], X[index(1),], beta.list[[tt]], kappa) - global_grad
    }
    # Solve surrogate loss minimization using Huber_GDBB, needs input of constant gradient shift and first machine's data.
    beta.temp = Huber_GDBB(y[index(1)], as.matrix(X[index(1),]), tau_const = kappa_const, distributed = T, beta0 = beta.list[[tt]], gradient_shift = grad_shift)$coefficients
    beta.temp[1] = median(sapply(1:m, function(x) huber.mean(y[index(x)]-X[index(x),-1]%*%beta.temp[-1])))
    loss.t = loss(y, X, beta.temp, tau)
    if (loss.t > loss.t_1){
      break
    }else{
      beta.list[[tt+1]] = beta.temp
    ...
    # beta.list[[tt+1]] = Huber_GDBB(y[index(1)], as.matrix(X[index(1),]), tau_const = kappa_const, distributed = T, beta0 = beta.list[[tt]], gradient_shift = grad_shift)
    
  }
  
  # # scale back beta
  # for (i in 1:length(beta.list)){
  #   beta.list[[i]][1] = beta.list[[i]][1]*sd_y + mean_y - sum(beta.list[[i]][-1]*mean_X/sd_X)*sd_y
  #   beta.list[[i]][2:length(beta.list[[i]])] = beta.list[[i]][2:length(beta.list[[i]])]/sd_X*sd_y
  # }
  # scale back beta
  for (l in 1:length(beta.list)){
    if(!univ){
  ...
  return(as.vector(beta.list[[length(beta.list)]]))
}


### Helper functions
tau_update_dd_high_d = function(y, X, beta){
  n = nrow(X)
  d = ncol(X) - 1
  res_square = as.vector(y - X%*%beta)^2
  s = sum(beta!=0)
